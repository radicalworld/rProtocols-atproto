...
export default function NeedEditorProfile({ rootId: propRootId, parentLineageId, isNew = false, onClose } = {}) {
    ...
    const release = useMemo(() => isNew ? { rootId: "new", version: "0.1.0", stage: "draft", title: "", description: "", purpose: "", language: "en", tags: [] } : fetchedRelease, [isNew, fetchedRelease]);
    const repo = useRepo();
    ...
    async function onPublish() {
        ...
        try {
            if (isNew) {
                ...
                const gen = await repo.getNeedByLineageId(nid) || await repo.getNeedByVersion(nid, "1.0");
                if (gen)
    ...
    return (_jsxs("div", { className: "mx-auto max-w-3xl p-6 space-y-4", children: [_jsxs("header", { className: "flex items-start justify-between", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-2xl font-semibold", children: isNew ? "Create Need" : "Edit Need" }), _jsx("div", { className: "text-sm text-gray-500 mt-1", children: isNew ? (_jsxs("span", { className: "flex items-center gap-2", children: [_jsx("span", { className: "font-semibold text-gray-700", children: "Parent Context:" }), _jsx("span", { className: "uppercase tracking-wider", children: parentLineageId || "None" })] })) : (_jsxs(_Fragment, { children: ["Editing version: ", _jsxs("span", { className: "font-mono", children: ["v", release.version] }), latest && latest !== release.version ? ` (latest is v${latest})` : ""] })) })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("button", { onClick: onPublish, disabled: !canEdit || saving, className: "rounded-lg bg-blue-600 px-4 py-1.5 text-sm font-medium text-white shadow hover:bg-blue-700 transition disabled:opacity-50", children: "Publish" }), !isNew && (_jsx("button", { onClick: onPromoteCandidate, disabled: !canEdit || saving, className: "rounded-lg bg-green-600 px-3 py-1.5 text-sm font-medium text-white shadow hover:bg-green-700 transition disabled:opacity-50", children: "Promote to Candidate" })), onClose && (_jsx("button", { onClick: onClose, className: "rounded p-2 text-sm text-gray-500 hover:bg-gray-100 transition-colors ml-2", children: "Cancel" }))] })] }), !canEdit && (_jsxs("div", { className: "rounded border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800", children: ["This version is ", _jsx("b", { children: release.stage }), ". Create a new draft to edit content."] })), _jsxs("label", { className: "block", htmlFor: "need-title", children: [_jsxs("div", { className: "text-sm font-medium text-gray-700", children: ["Title ", isNew ? "" : _jsx("span", { className: "text-xs text-gray-400 font-normal ml-1", children: "(Immutable via Lineage)" })] }), _jsx("input", { id: "need-title", name: "title", className: `mt-1 w-full rounded border p-2 ${!isNew ? "bg-gray-50 text-gray-500 cursor-not-allowed" : ""}`, value: form.title, onChange: (e) => setForm({ ...form, title: e.target.value }), disabled: !isNew })] }), _jsxs("label", { className: "block", htmlFor: "need-language", children: [_jsx("div", { className: "text-sm font-medium text-gray-700", children: "Language" }), _jsx("input", { id: "need-language", name: "language", className: "mt-1 w-full rounded border p-2", placeholder: "e.en", value: form.language, onChange: (e) => setForm({ ...form, language: e.target.value }), disabled: !canEdit || saving })] }), _jsxs("label", { className: "block", htmlFor: "need-tags", children: [_jsx("div", { className: "text-sm font-medium text-gray-700", children: "Tags (comma separated)" }), _jsx("input", { id: "need-tags", name: "tags", className: "mt-1 w-full rounded border p-2", placeholder: "tag1, tag2", value: form.tags, onChange: (e) => setForm({ ...form, tags: e.target.value }), disabled: !canEdit || saving })] }), _jsxs("label", { className: "block", htmlFor: "need-purpose", children: [_jsx("div", { className: "text-sm font-medium text-gray-700", children: "Purpose" }), _jsx("textarea", { id: "need-purpose", name: "purpose", className: "mt-1 w-full rounded border p-2 h-24", value: form.purpose, onChange: (e) => setForm({ ...form, purpose: e.target.value }), disabled: !canEdit || saving })] }), _jsxs("label", { className: "block", htmlFor: "need-description", children: [_jsx("div", { className: "text-sm font-medium text-gray-700 mb-1", children: "Description (Markdown)" }), _jsx("div", { className: !canEdit || saving ? "opacity-60 pointer-events-none" : "", children: _jsx(TiptapEditor, { content: form.description, onChange: (value) => setForm({ ...form, description: value }) }) })] }), msg && _jsx("div", { className: "text-sm text-gray-500", children: msg })] }));
}
