...
export default function ProtocolEditorProfile({ protocolId: propRootId, parentNeedId, isNew = false, onClose } = {}) {
    ...
    const draft = useMemo(() => isNew ? { rootId: "new", version: "0.1.0", stage: "draft", title: "", summary: "", body: "", language: "en", tags: [] } : fetchedDraft, [isNew, fetchedDraft]);
    ...
    if (!draft)
        return _jsx("div", { className: "p-6 text-sm text-gray-500", children: "No protocol layout available." });
    if (!isInitialized)
    ...
    async function onUnifiedSave() {
        ...
        try {
            ...
            if (isNew) {
                ...
                if (gen)
                    await repo.follow(gen.lineageId);
