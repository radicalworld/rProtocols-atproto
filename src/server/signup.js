...
app.post("/api/signup", async (req, res) => {
    ...
    if (data.accessJwt)
        res.cookie("accessJwt", data.accessJwt, cookieOpts);
    if (data.refreshJwt)
        res.cookie("refreshJwt", data.refreshJwt, { ...cookieOpts, maxAge: 60 * 60 * 24 * 30 });
...
app.post("/api/login", async (req, res) => {
    ...
    const data = await r.json();
    if (!r.ok)
        return res.status(r.status).json({ error: data?.message ?? "Login failed", details: data });
    const cookieOpts = {
        httpOnly: true,
        sameSite: "lax",
        secure: true,
