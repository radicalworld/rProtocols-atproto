...
export class AtprotoAdapter {
    ...
    async createNeed(payload, forceId) {
        if (!this.viewerDid)
            throw new Error("Not authenticated");
        const rootId = forceId || payload.title.toLowerCase().replace(/\s+/g, "-");
        await this.updateNeedDraft(rootId, "0.1.0", { ...payload, parentLineageId: payload.parentLineageId ?? null });
        return rootId;
    }
    async createProtocol(payload, forceId) {
        if (!this.viewerDid)
            throw new Error("Not authenticated");
        const rootId = forceId || payload.title.toLowerCase().replace(/\s+/g, "-");
            ...
            await this.agent.com.atproto.repo.putRecord({
                ...
                record: {
                    ...
                    members: {
                        ...
                        protocols: payload.includeProtocols?.map((p) => ({ uri: `at://${this.viewerDid}/org.rp.protocol/${p.lineageId || p.uri || ""}`, cid: "mock" })) || []
                    },
                    tags: payload.tags ?? []
                }
            });
            ...
            await this.agent.com.atproto.repo.putRecord({
                ...
                record: {
                    ...
                    members: {
                        ...
                        protocols: patch.includeProtocols?.map((p) => ({ uri: `at://${this.viewerDid}/org.rp.protocol/${p.lineageId || p.uri || ""}`, cid: dummyCid })) || []
                    },
                    tags: patch.tags ?? []
                }
            });
